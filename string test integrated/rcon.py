...
#from gen_rtrain import rtrain,rref
#from gen_rtest2 import rtest
...
#             batch_x, batch_y = rtrain(N,ans)
            ...
            if step % display_step == 0 or step == 1:
                ...
                loss, acc = sess.run([loss_op, accuracy], feed_dict={X: batch_x,
                                                                     Y: batch_y[:,0:m],
                                                                     keep_prob: 1.0})
                los, ac = sess.run([loss_op, accuracy], feed_dict={X: batch_x,
                                                                     Y: batch_y[:,0:m],
                                                                     keep_prob: 1.0})
                print("Step " + str(step) + ", Minibatch Loss= " + \
                     ...
                     "{:.3f}".format(ac))
                if ac_max<ac:
                    save_path = saver.save(sess, 'model/model2'+str(m)+'.ckpt')
                    ac_max=ac
                if ac==1:
                    if step>5:
                        Num=1
...
# #                             keep_prob: 1.0})
        ...
        acc=np.zeros(m)
        for i in range (0,m):
            acc[i]=sess.run(accuracy, feed_dict={X: x[i*10:i*10+10,:],
