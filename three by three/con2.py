...
from loadone2 import load_sample as load
from convo import conv2d,maxpool2d,conv_net
def train_cov(A,n,a,bs):
    # Training Parameters
    Num=0
    learning_rate = 0.001
    num_steps = n
    ...
    weights = {
        ...
        'wc1': tf.Variable(tf.random_normal([5, 5, 1, a])),
        # 5x5 conv, 32 inputs, 64 outputs
        'wc2': tf.Variable(tf.random_normal([5, 5, a, 2*a])),
        # fully connected, 7*7*64 inputs, 1024 outputs
        'wd1': tf.Variable(tf.random_normal([7*7*2*a, 1024])),
