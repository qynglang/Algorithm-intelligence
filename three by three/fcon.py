...
def fullnet(fc1, weights, biases, dropout):
    ...
    return out
# Training Parameters
def train_fcon(pic,weight1,weight2,ans,n,a,bs):
    Num=0
    learning_rate = 0.001
    num_steps = n
    batch_size = bs
    ...
    # Network Parameters
    num_input = 7*7*2*a*3*weight1.shape[4] # MNIST data input (img shape: 28*28)
    ...
    weights = {
        ...
        'wd1': tf.Variable(tf.random_normal([7*7*2*a*3*weight1.shape[4], 1024])),
    ...
    with tf.Session() as sess:
        ...
        for step in range(1, num_steps+1):
            batch_x, batch_y = pic3_2(pic,weight1,weight2,a)
