...
def fullnet1(fc1, weights, biases, dropout):
    ...
    return fc1,out
# Training Parameters
def train_fcon1(pic,weight1,weight2,n,bs,l):
    Num=0
    learning_rate = 0.001
    num_steps = n
    batch_size = bs
    ...
    # Network Parameters
    num_input = 1204 # MNIST data input (img shape: 28*28)
    ...
    weights = {
        ...
        'wd1': tf.Variable(tf.random_normal([1204, 1024])),
    ...
    # Construct model
    fc1,logits = fullnet1(X, weights, biases, keep_prob)
    ...
    with tf.Session() as sess:
        ...
        for step in range(1, num_steps+1):
            ...
            sess.run(train_op, feed_dict={X: np.hstack((batch_x,l)), Y: batch_y, keep_prob: dropout})
            if step % display_step == 0 or step == 1:
                # Calculate batch loss and accuracy
                loss, acc = sess.run([loss_op, accuracy], feed_dict={X: np.hstack((batch_x,l)),
                                                                     Y: batch_y,
                                                                     keep_prob: 1.0})
...
#                             keep_prob: 1.0})
        ...
        l=fc1.eval(feed_dict={X: np.hstack((batch_x,l)),
                            Y: batch_y,
                            keep_prob: 1.0})
    return w,b,l,Num