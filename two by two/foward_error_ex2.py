...
from fcon_excape_c4 import train_fcon2
...
def foward_error (n,m,ans,N_str):
    ...
    for i in range (0,tr):
        ...
        if Num==1:
            T=T+1
    for i in range (0,tr):
        wf3[:,:,i],bf3[:,i],l2,Num=train_fcon2(data[:,:,:,n],w1[:,:,:,:,i],w2[:,:,:,:,i],50,8,l1,l)
        if Num==1:
            T=T+1
    #print(l1.shape)
#     EE=np.zeros((6,5))
...
#     np.save('Data/'+str(N_str)+'Termination.npy',T)
    np.save('Data/'+str(N_str)+'weight1c_ex_f.npy',w1)
    np.save('Data/'+str(N_str)+'weight2c_ex_f.npy',w2)
    np.save('Data/'+str(N_str)+'weightf1_ex_f.npy',wf1)
    np.save('Data/'+str(N_str)+'biasf1_ex_f.npy',bf1)
    np.save('Data/'+str(N_str)+'weightf2_ex_f.npy',wf2)
    np.save('Data/'+str(N_str)+'biasf2_ex_f.npy',bf2)
    np.save('Data/'+str(N_str)+'weightf3_ex_f.npy',wf3)
    np.save('Data/'+str(N_str)+'biasf3_ex_f.npy',bf3)