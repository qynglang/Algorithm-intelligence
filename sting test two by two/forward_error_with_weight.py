...
def foward_error (n,m,ans,N_str):
    ...
    w1=np.load('Data/'+str(N_str)+'weight1c.npy')
    w2=np.load('Data/'+str(N_str)+'weight2c.npy')
    wf1=np.load('Data/'+str(N_str)+'weightf1.npy')
    bf1=np.load('Data/'+str(N_str)+'biasf1.npy')
    wf2=np.load('Data/'+str(N_str)+'weightf2.npy')
    bf2=np.load('Data/'+str(N_str)+'biasf2.npy')
    wf3=np.load('Data/'+str(N_str)+'weightf3.npy')
    bf3=np.load('Data/'+str(N_str)+'biasf3.npy')
    O2=np.zeros((6,1024))
    O3=np.zeros((6,512))
    O4=np.zeros((6,256))
    E=np.zeros(6)
    ...
    for j in range (0,w1.shape[4]):
        ...
        O1_2=np.dot(O1.reshape(1,1024),wf2[:,:,j])+bf2[:,j]
        for i in range (0,6):
            O3[i,:]=np.dot(O2[i,:].reshape(1,1024),wf2[:,:,j])+bf2[:,j]
        
        O1_3=np.dot(O1_2.reshape(1,512),wf3[:,:,j])+bf3[:,j]
        for i in range (0,6):
            O4[i,:]=np.dot(O3[i,:].reshape(1,512),wf3[:,:,j])+bf3[:,j]
        for i in range (0,6):
            E[i]=abs(O2[i,:]-O1).sum()
        EE[:,j]=E
        for i in range (0,6):
