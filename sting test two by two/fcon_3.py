...
def train_fcon(m,n):
    ...
    # Network Parameters
    num_input = n # MNIST data input (img shape: 28*28)
    ...
    weights = {
        ...
        'wd1': tf.Variable(tf.random_normal([n, 1024])),
    ...
    with tf.Session() as sess:
        ...
        for step in range(1, num_steps+1):
            ...
            batch_x, batch_y = load([n],128,m)
            batch_x=batch_x.reshape(128,n)
            v_x, v_y = load([n],128,m)
            v_x=v_x.reshape(128,n)
            ...
            if step % display_step == 0 or step == 1:
                ...
                if ac_max<ac:
                    ac_max=ac
                    save_path = saver.save(sess, 'model2/model'+str(m)+'.ckpt')
        ...
        #step=np.argmax(ac)
        saver.restore(sess, 'model2/model'+str(m)+'.ckpt')

        test,ans=load_test([n],100,m)
        test=test.reshape(100,n)
        acc=np.zeros(m)
        for i in range (0,m):
             acc[i]=sess.run(accuracy, feed_dict={X: test[i*10:(i+1)*10,:].reshape(10,n),
