...
def fullnet2(fc1, weights, biases, dropout):
    ...
    return fc1,out
# Training Parameters
def train_fcon2(pic,weight1,weight2,n,bs,l1,l):
    Num=0
    learning_rate = 0.001
    num_steps = n
    batch_size = bs
    ...
    # Network Parameters
    num_input = 2048 # MNIST data input (img shape: 28*28)
    ...
    weights = {
        ...
        'wd1': tf.Variable(tf.random_normal([2048, 1024])),
    ...
    with tf.Session() as sess:
        ...
        for step in range(1, num_steps+1):
            ...
            sess.run(train_op, feed_dict={X: np.hstack((l1,l)), Y: batch_y, keep_prob: dropout})
            if step % display_step == 0 or step == 1:
                # Calculate batch loss and accuracy
                loss, acc = sess.run([loss_op, accuracy], feed_dict={X: np.hstack((l1,l)),
                                                                     Y: batch_y,
                                                                     keep_prob: 1.0})
...
#                             keep_prob: 1.0})
        ...
        l=fc1.eval(feed_dict={X: np.hstack((l1,l)),
                            Y: batch_y,
                            keep_prob: 1.0})
    return w,b,l,Num